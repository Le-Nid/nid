...
La documentation est générée avec [VitePress](https://vitepress.dev/) et supporte le français et l'anglais.

```bash
# Serveur de dev docs (depuis la racine du projet)
npm run docs:dev
...
Les fichiers de doc se trouvent dans `docs/fr/` (français) et `docs/en/` (anglais).
