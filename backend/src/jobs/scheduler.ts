  ...
  async function tick() {
    ...
    try {
      ...
      const rules = await db`
        SELECT r.id, r.gmail_account_id, r.schedule, r.last_run_at,
               ga.email as account_email
        FROM rules r
        JOIN gmail_accounts ga ON r.gmail_account_id = ga.id
        WHERE r.is_active = true
          AND r.schedule IS NOT NULL
      `

      const now = new Date()

      for (const rule of rules) {
        if (shouldRun(rule.schedule, rule.last_run_at, now)) {
          await enqueueJob('run_rule', {
            accountId: rule.gmail_account_id,
            ruleId: rule.id,
          })
          console.info(`[Scheduler] Enqueued rule ${rule.id} (${rule.account_email})`)
        }
      }
    } catch (err) {
