...
import { z } from 'zod'
import { getRules, getRule, createRule, updateRule, deleteRule, runRule, buildGmailQuery } from '../rules/rules.service'
import { enqueueJob } from '../jobs/queue'
import { listMessages } from '../gmail/gmail.service'

const conditionSchema = z.object({
  field:    z.enum(['from', 'to', 'subject', 'has_attachment', 'size_gt', 'size_lt', 'label']),
...
const ruleCreateSchema = z.object({
  ...
  action:      actionSchema,
  schedule:    z.enum(['hourly', 'daily', 'weekly', 'monthly']).nullable().optional(),
...
export async function rulesRoutes(app: FastifyInstance) {
  ...
  app.get('/:accountId/:ruleId', auth, async (request, reply) => {
    const { accountId, ruleId } = request.params as { accountId: string; ruleId: string }
  ...
  app.post('/:accountId', auth, async (request, reply) => {
    ...
    const rule = await createRule(accountId, dto)
  ...
  app.put('/:accountId/:ruleId', auth, async (request) => {
    ...
    return updateRule(ruleId, accountId, dto)
  })
  ...
  app.post('/:accountId/:ruleId/run', auth, async (request, reply) => {
    ...
    return reply.code(202).send({ jobId: job.id, message: 'Rule execution enqueued' })
  })
