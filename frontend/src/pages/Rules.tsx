...
import {
  Table, Button, Tag, Space, Typography, Switch, Popconfirm,
  Tooltip, notification, message, Card, Badge, Empty
} from 'antd'
import {
...
import { Rule, ACTION_LABELS, CONDITION_FIELD_LABELS, CONDITION_OPERATOR_LABELS, SCHEDULE_OPTIONS } from '../types/rules'
...
export default function RulesPage() {
  ...
  const openEdit = (rule: Rule) => { setEditingRule(rule); setModalOpen(true) }
        ...
        <Space direction="vertical" size={2}>
          {conditions.map((c, i) => (
            <Tag key={i} style={{ fontSize: 11 }}>
              ...
              <Text type="secondary">{CONDITION_OPERATOR_LABELS[c.operator]}</Text>{' '}
              {c.field !== 'has_attachment' && <strong>{String(c.value)}</strong>}
            </Tag>
          ))}
        </Space>
      ),
  ...
  return (
    <div>
      ...
      <Space style={{ marginBottom: 16 }} align="center">
        <Title level={3} style={{ margin: 0 }}>🤖 Règles automatiques</Title>
      ...
      >
        <Text style={{ fontSize: 13 }}>
          Les règles analysent votre boîte Gmail et appliquent des actions automatiquement.
          Utilisez <strong>Prévisualiser</strong> pour voir combien de mails seront affectés avant de sauvegarder.
          Les actions bulk sont toujours exécutées via des <strong>jobs asynchrones</strong>.
        </Text>
      </Card>
